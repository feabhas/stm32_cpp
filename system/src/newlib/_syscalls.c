...
static struct fdent* findslot(int fd)
{
  ...
  if ((unsigned int)fd >= MAX_OPEN_FILES) { return NULL; }

  /* User file descriptor is open? */
  if (openfiles[fd].handle == -1) { return NULL; }
...
void initialise_monitor_handles(void)
{
  ...
  if (monitor_stderr == -1) { monitor_stderr = monitor_stdout; }
...
int _swilseek(int fd, int ptr, int dir)
{
  ...
  if (dir == SEEK_CUR) {
    ...
    if (ptr < 0) {
      errno = EINVAL;
      if ((pfd->pos > 0) && (ptr > 0)) { errno = EOVERFLOW; }
      return -1;
    }
    dir = SEEK_SET;
  }
...
#ifdef O_BINARY
  if (flags & O_BINARY) { aflags |= 1; }
#endif

  /* In O_RDONLY we expect aflags == 0. */

  if (flags & O_RDWR) { aflags |= 2; }
...
int __attribute__((weak)) _stat(const char* fname, struct stat* st)
{
  ...
  if ((fd = _open(fname, O_RDONLY)) == -1) { return -1; }
...
int _system(const char* s)
{
  ...
  if (!s) { return 1; /* maybe there is a shell available? we can hope. :-P */ }
  block[0] = (uint32_t)s;
  block[1] = strlen(s);
